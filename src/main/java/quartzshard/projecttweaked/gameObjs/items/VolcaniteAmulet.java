...
import main.java.quartzshard.projecttweaked.utils.FluidHelper;
...
public class VolcaniteAmulet extends ItemPE implements IProjectileShooter, IBauble, IPedestalItem, IFireProtector
{
	...
	{
		if (!world.isRemote && ProjectTwEakedConfig.pedestalCooldown.volcanitePedCooldown != -1)
		{
			...
			if (tile.getActivityCooldown() == 0)
			{
				...
				tile.setActivityCooldown(ProjectTwEakedConfig.pedestalCooldown.volcanitePedCooldown);
			}
			else
			{
	...
	public List<String> getPedestalDescription()
	{
		...
		if (ProjectTwEakedConfig.pedestalCooldown.volcanitePedCooldown != -1)
		{
			list.add(TextFormatting.BLUE + I18n.format("pe.volcanite.pedestal1"));
			list.add(TextFormatting.BLUE + I18n.format("pe.volcanite.pedestal2", MathUtils.tickToSecFormatted(ProjectTwEakedConfig.pedestalCooldown.volcanitePedCooldown)));
		}
		return list;
	}
