...
import main.java.quartzshard.projecttweaked.gameObjs.gui.InventoryCraftingArcaneTablet;
...
/**
 * Credit to LatvianModder for ProjectTwEakedX tablet this is based on
 */
public class TweakedTransmutationContainer extends ContainerTableBase
{
	...
	public TweakedTransmutationContainer(EntityPlayer player, TransmutationInventory inventory, @Nullable EnumHand hand)
	{
		super(player, inventory, hand);
		...
		addSlotToContainer(new SlotCrafting(player, craftMatrix, craftResult, 0, -23, 75)
		{
			...
			protected void onCrafting(ItemStack stack)
			{
				...
				if (ProjectTwEakedAPI.getEMCProxy().hasValue(stack) && EXUtils.addKnowledge(player, playerData, EXUtils.fixOutput(stack)) == 2 && knowledgeUpdate != null)
				{
	...
	private boolean transferFromTablet(int i, ItemStack[] possibilities)
	{
		...
		{
			Arrays.sort(possibilities, Comparator.comparingLong(o -> ProjectTwEakedAPI.getEMCProxy().getValue(o)));
		}
	...
	private boolean transferFromInventory(int i, ItemStack[] possibilities)
	{
		...
		{
			ItemStack fixedp = EXUtils.fixOutput(possibility);

			for (int j = 0; j < player.inventory.getSizeInventory(); ++j)
			{
				ItemStack stack = player.inventory.getStackInSlot(j);

				if (ItemStack.areItemStacksEqualUsingNBTShareTag(fixedp, EXUtils.fixOutput(stack)))
				{
					ItemStack slotItem = craftMatrix.getStackInSlot(i);
	...
	public void clearCraftingMatrix()
	{
		for (int i = 0; i < craftMatrix.getSizeInventory(); i++)
		{
			...
			if (!stack.isEmpty())
			{
				if (ProjectTwEakedConfig.difficulty.covalenceLoss >= 1D && ProjectTwEakedAPI.getEMCProxy().hasValue(stack))
				{
					int k = EXUtils.addKnowledge(player, playerData, EXUtils.fixOutput(stack));

					if (k > 0)
					{
						EXUtils.addEMC(transmutationInventory, ProjectTwEakedAPI.getEMCProxy().getValue(stack) * stack.getCount());
