...
import main.java.quartzshard.projecttweaked.network.packets.CheckUpdatePKT;
import main.java.quartzshard.projecttweaked.network.packets.SyncCovalencePKT;
...
public class PlayerEvents
{
	...
	{
		NBTTagCompound bags = evt.getOriginal().getCapability(ProjectTwEakedAPI.ALCH_BAG_CAPABILITY, null).serializeNBT();
		evt.getEntityPlayer().getCapability(ProjectTwEakedAPI.ALCH_BAG_CAPABILITY, null).deserializeNBT(bags);

		NBTTagCompound knowledge = evt.getOriginal().getCapability(ProjectTwEakedAPI.KNOWLEDGE_CAPABILITY, null).serializeNBT();
		evt.getEntityPlayer().getCapability(ProjectTwEakedAPI.KNOWLEDGE_CAPABILITY, null).deserializeNBT(knowledge);
	}
	...
	{
		evt.player.getCapability(ProjectTwEakedAPI.KNOWLEDGE_CAPABILITY, null).sync((EntityPlayerMP) evt.player);
		evt.player.getCapability(ProjectTwEakedAPI.ALCH_BAG_CAPABILITY, null).sync(null, (EntityPlayerMP) evt.player);
	}
	...
	public static void playerChangeDimension(net.minecraftforge.fml.common.gameevent.PlayerEvent.PlayerChangedDimensionEvent event)
	{
		...
		event.player.getCapability(ProjectTwEakedAPI.KNOWLEDGE_CAPABILITY, null).sync((EntityPlayerMP) event.player);
		event.player.getCapability(ProjectTwEakedAPI.ALCH_BAG_CAPABILITY, null).sync(null, (EntityPlayerMP) event.player);
	...
	public static void playerConnect(net.minecraftforge.fml.common.gameevent.PlayerEvent.PlayerLoggedInEvent event)
	{
		...
		IKnowledgeProvider knowledge = player.getCapability(ProjectTwEakedAPI.KNOWLEDGE_CAPABILITY, null);
		knowledge.sync(player);
		PlayerHelper.updateScore(player, PlayerHelper.SCOREBOARD_EMC, MathHelper.floor(knowledge.getEmc()));

		player.getCapability(ProjectTwEakedAPI.ALCH_BAG_CAPABILITY, null).sync(null, player);

		PacketHandler.sendTo(new SyncCovalencePKT(ProjectTwEakedConfig.difficulty.covalenceLoss, ProjectTwEakedConfig.difficulty.covalenceLossRounding), player);
	...
	public static void pickupItem(EntityItemPickupEvent event)
	{
		...
		IItemHandler handler = player.getCapability(ProjectTwEakedAPI.ALCH_BAG_CAPABILITY, null)
