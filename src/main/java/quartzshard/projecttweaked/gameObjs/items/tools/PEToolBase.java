...
public abstract class PEToolBase extends ItemMode
{
	...
	protected void clearOdAOE(World world, ItemStack stack, EntityPlayer player, String odName, long emcCost, EnumHand hand)
	{
		...
		if (charge == 0 || world.isRemote || ProjectTwEakedConfig.items.disableAllRadiusMining)
		{
			return;
		}
	...
	protected void digBasedOnMode(ItemStack stack, World world, Block block, BlockPos pos, EntityLivingBase living)
	{
		...
		if (!ProjectTwEakedConfig.items.disableAllRadiusMining) {
			switch (mode) {
				case 1: // 3x Tallshot
	...
	{
		if (world.isRemote || this.getCharge(stack) == 0 || ProjectTwEakedConfig.items.disableAllRadiusMining)
		{
			return;
		}
	...
	{
		if (player.getEntityWorld().isRemote || ProjectTwEakedConfig.items.disableAllRadiusMining)
		{
			return;
		}
