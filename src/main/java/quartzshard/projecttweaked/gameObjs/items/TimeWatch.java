...
public class TimeWatch extends ItemPE implements IModeChanger, IBauble, IPedestalItem, IItemCharge
{
	...
	public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, @Nonnull EnumHand hand)
	{
		...
		if (!world.isRemote)
		{
			if (!ProjectTwEakedConfig.items.enableTimeWatch)
			{
	...
	private void speedUpTileEntities(World world, int bonusTicks, AxisAlignedBB bBox)
	{
		...
		List<String> blacklist = Arrays.asList(ProjectTwEakedConfig.effects.timeWatchTEBlacklist);
	...
	private void speedUpRandomTicks(World world, int bonusTicks, AxisAlignedBB bBox)
	{
		...
		List<String> blacklist = Arrays.asList(ProjectTwEakedConfig.effects.timeWatchBlockBlacklist);
	...
	public void updateInPedestal(@Nonnull World world, @Nonnull BlockPos pos)
	{
		...
		if (!world.isRemote && ProjectTwEakedConfig.items.enableTimeWatch)
		{
			TileEntity te = world.getTileEntity(pos);
			if (te instanceof DMPedestalTile)
			{
				AxisAlignedBB bBox = ((DMPedestalTile) te).getEffectBounds();
				if (ProjectTwEakedConfig.effects.timePedBonus > 0) {
					speedUpTileEntities(world, ProjectTwEakedConfig.effects.timePedBonus, bBox);
					speedUpRandomTicks(world, ProjectTwEakedConfig.effects.timePedBonus, bBox);
				}

				if (ProjectTwEakedConfig.effects.timePedMobSlowness < 1.0F) {
					slowMobs(world, bBox, ProjectTwEakedConfig.effects.timePedMobSlowness);
				}
			}
		}
	}
	...
	public List<String> getPedestalDescription()
	{
		...
		if (ProjectTwEakedConfig.effects.timePedBonus > 0) {
			list.add(TextFormatting.BLUE + I18n.format("pe.timewatch.pedestal1", ProjectTwEakedConfig.effects.timePedBonus));
		}
		if (ProjectTwEakedConfig.effects.timePedMobSlowness < 1.0F)
		{
			list.add(TextFormatting.BLUE + I18n.format("pe.timewatch.pedestal2", ProjectTwEakedConfig.effects.timePedMobSlowness));
		}
		return list;
	}
